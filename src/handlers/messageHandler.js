...
async function handleMessage(message) {
    try {
        ...
        if (result.type === 'data_extraction') {
            ...
            console.log(`[DEBUG] Step 1: Executing Text Strategy for "${result.content}"`);
            const response = await textStrategy.execute(result.content, message, user, userContext);
            console.log(`[DEBUG] Step 2: Strategy Returned. Type: ${response.type}`);
            ...
            if (response.type === 'ai_response' || response.type === 'tool_response') {
                ...
                console.log(`[DEBUG] Step 3: Processing Response content: ${text.substring(0, 50)}...`);
                ...
                try {
                    ...
                    console.log(`[DEBUG] Step 4: JSON Parsed Successfully`);

                    // 3. Validação com Zod
                    const validation = AIResponseSchema.safeParse(parsedRaw);

                    if (validation.success) {
                        console.log(`[DEBUG] Step 5: Validation Success`);
                        ...
                        console.log(`[DEBUG] Step 5: Validation Failed`);
                    ...
                    console.log(`[DEBUG] Step 4: JSON Parse Failed (Treating as plain text). Error: ${e.message}`);
                    ...
                    if (jsonStr.includes('"gastos"')) {
                        ...
                        console.log(`[DEBUG] Step 6: Sending Plain Text Reply`);
                        await message.reply(text);
                        console.log(`[DEBUG] Step 7: Reply Sent`);
                    }
                }
        ...
        // ADIÇÃO: Tentar avisar o usuário se possível
