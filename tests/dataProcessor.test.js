...
describe('DataProcessor - Helper Functions', () => {
    ...
    describe('_generatePayload', () => {
        test('deve gerar payload com status confirmed para alta confiança', () => {
            const validItems = [
                {
                    valor: 10.50,
            ...
            const result = _generatePayload(validItems, embeddings, userId, data);
        ...
        test('deve gerar payload com status pending_review para baixa confiança', () => {
            const validItems = [
                {
                    valor: 20,
                    valor_original: 20,
        ...
        test('deve usar confidence_score padrão 1.0 quando não fornecido', () => {
            const validItems = [{
                valor: 5,
                valor_original: 5,
                moeda_original: 'BRL',
        ...
        test('deve incluir prompt_version no payload', () => {
            const validItems = [{
                valor: 15,
                valor_original: 15,
                moeda_original: 'BRL',
            ...
            expect(result.payload[0].prompt_version).toBe('v2_experimental');
        });

        test('deve usar prompt_version padrão quando não fornecido', () => {
            const validItems = [{
                valor: 25,
                valor_original: 25,
                moeda_original: 'BRL',
            ...
            expect(result.payload[0].prompt_version).toBe('v1_stable');
        });

        test('deve mapear corretamente todos os campos do payload', () => {
            const validItems = [{
                valor: 35.75,
                valor_original: 30,
                moeda_original: 'USD',
            ...
            expect(item.valor).toBe(35.75);
            expect(item.valor_original).toBe(30);
            expect(item.moeda_original).toBe('USD');
            expect(item.taxa_cambio).toBe(5.25);
            expect(item.descricao).toBe('Produto Importado');
            expect(item.categoria).toBe('Compras Online');
            expect(item.tipo).toBe('despesa');
            expect(item.embedding).toEqual([0.7, 0.8, 0.9]);
            expect(item.confidence_score).toBe(0.8);
        });
    });
});
